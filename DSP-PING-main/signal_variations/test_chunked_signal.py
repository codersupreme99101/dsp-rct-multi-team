...
def generate_test_signal(): # Computed signal parameters
        ...
        max_timer=int(np.floor(len_signal/time_chunk))+1
        counter_freqs=0
        ...
        for i in range(max_timer):
            ...
            if int((i+1)*time_chunk)>len_signal:
                arr_ch=np.linspace(int(i*time_chunk), len_signal, int(len_signal-int(i*time_chunk)))
                chunked_signal[int(i*time_chunk):len_signal]= ping_amplitude* np.cos(f_t_arr[counter_freqs] / 2 * np.pi* arr_ch) + 1j * np.sin(f_t_arr[counter_freqs] / 2 * np.pi* arr_ch)

            else:
                arr_ch=np.linspace(int(i*time_chunk), int((i+1)*time_chunk), int(time_chunk))
                chunked_signal[int(i*time_chunk):int((i+1)*time_chunk)]= ping_amplitude* np.cos(f_t_arr[counter_freqs] / 2 * np.pi* arr_ch) + 1j * np.sin(f_t_arr[counter_freqs] / 2 * np.pi* arr_ch)
